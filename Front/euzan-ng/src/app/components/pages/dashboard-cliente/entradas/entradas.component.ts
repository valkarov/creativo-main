...
import { TallerClientesService, TalleresService, TalleresPagoService } from 'src/app/services/talleres.service';
  ...
  downloadPdf(taller:Taller) {
        ...
        this.emService.get("User",taller.IdEntrepreneurship).subscribe({
          next:(data) =>{
            ...
            this.clientService.get("User", this.cookieService.get("cookieCLIENTE")).subscribe({
              next:(data) =>{
                console.log(data)
                ...
                doc.setFontSize(16);
                doc.text('Emprendimiento:', margins.left, currentY + 10);
            
                autoTable(doc, {
                  startY: currentY + 20,
                  head: [['Campo', 'Valor']],
                  body: [
                    ['ID Emprendimiento', emprendimiento.IdEntrepreneurship.toString()],
