...
import { TallerClientesService, TalleresPagoService, TalleresService } from 'src/app/services/talleres.service';
...
export class VerTalleresComponent {
  ...
  registrarPago(taller:Taller) {
    ...
    this.emService.get("User", taller.IdEntrepreneurship).subscribe({
      next:(data) => {
        console.log(data)
        ...
        Swal.fire({
          ...
          preConfirm: () => {
            ...
            if (!/^\d{4}$/.test(input1)) {
              Swal.showValidationMessage('Debe ingresar exactamente 4 dígitos numéricos');
              return false;
            }
          ...
          if (result.isConfirmed) {
            console.log('Dígitos Transacción:', result.value?.input1);
            console.log('Nombre Propietario:', result.value?.input2);
            ...
            this.pagoService.add(pago).subscribe({
              next:(data) =>{
                console.log(data)
                this.service.successMessage("Te has suscrito al Taller, en cuanto el emprendimiento verifique tu pago, llegarán las entradas a tu correo", "/ver-talleres");
              }, error:(err) =>{
                console.log(err)
              }
            })
